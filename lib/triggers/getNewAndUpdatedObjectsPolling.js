...
exports.process = async function process(msg, cfg) {
  ...
  if ((startTime && !dateRegExp.test(startTime))
    ...
    throw new Error('Please enter a date in the format "YYY-MM-DD[T]HH:MM:SS[Z]" where [] wraps a fixed character value');
  }
  ...
  const resultsList = client.makeRequest({
    url: queryString,
    method: 'GET',
  });
  ...
  resultsList.forEach((obj) => { obj[polledTimestamp] = new Date(obj[polledTimestamp]); })
    .filter((obj) => obj[polledTimestamp] >= pollingStartTime
