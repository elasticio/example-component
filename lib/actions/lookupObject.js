...
exports.process = async function process(msg, cfg) {
  ...
  if (linkedObjectToPopulate) {
    ...
    // Test for children:
    if (linkedObjectToPopulate.endsWith('s')) {
      linkedObjectParam = `_embed=${encodeURIComponent(linkedObjectToPopulate)}`;
    } else {
      linkedObjectParam = `_expand=${encodeURIComponent(linkedObjectToPopulate)}`;
    }
...
exports.getMetaModel = async function getMetaModel(cfg) {
  ...
  //console.log(description[cfg.lookupCriteria]);

  const uniqueCriteria = description[cfg.lookupCriteria];
